...
- **同日追加④（header logo 換文字）**：#hbrand 的 .brandlogo（61px logo mask）拿掉，改兩行置中文字「Ratio.」（Fraunces 21px 粗體）＋「Coffee is all we do.」（.brandtag 11px muted）；.brandlogo CSS 刪除、.brand 加 text-align:center＋padding 10px 0。**.bglogo 背景浮水印沒動**。驗證：截圖排版 ✓、購物車/員工狀態字右側不擠 ✓。
