...
- **同日追加③（訂閱卡 Blend 形象圖）**：老闆給咖啡店插畫圖 → `new/sub-blend.jpg`（原檔 ~/Downloads/ratio-cafe-960x960.jpg 1.5M，sips 品質 65 壓到 390K）；shopSubCardHTML 說明文字後插 `cur.plan==='blend'` 條件圖（width:100% 圓角 12、lazy）。Single origin 沒圖（老闆之後給再加一行同款）。驗證：Subscription tab blend 出圖＋排版 ✓、single 無圖 ✓。
