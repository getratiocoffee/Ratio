...
- **同日追加九（老闆點名）**：Customers 區新磁貼 **Post Label**（data-t 'addrlabel'）＝收件人貼紙 .lbx（Brother）——**重用 blend-label.lbx 範本＋lblFitBlock**：名字塊→收件人大寫、成分行→地址行1、comment→地址行2、風味行→Suburb State Postcode、date 行→電話、NET 清空。挑客人自動帶 `customers.address`（**手動 {line1,suburb,postcode} 與 Square {address_line_1,locality,…} 兩格式都認**，custAddrParts 歸一）＋phone，欄位可手改；檔名 POST_名字.lbx；logAct。驗證：兩格式帶入 ✓、zip 內 label.xml 收件字樣正確＋範本原字全換 ✓。
