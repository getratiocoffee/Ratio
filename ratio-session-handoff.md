...
- **追加（老闆點名上層同豆收抽屜）**：Shop 層改 name+process 聚合一行（合計 kg＋N batches＋組含 pool 批就亮框 ‹new›；組序＝最早入池順序）→ 點行展開各批明細（☐/鮮度點/日期/INV# 或 ‹new›/kg/↓ 全在明細行）；`TRFO` 手風琴一次一組、↓ 加 stopPropagation。驗證：DK 3 批聚 7kg ‹new›、收合無明細/展開 3 行帶各自 INV#、手風琴切組、明細 ↓ 退回寫入鏈 ✓、截圖 ✓。
