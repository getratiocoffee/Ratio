...
- **同 session 追加（線上資料操作）**：①Live→On shelf 文案改名（672154e，四處：Coffee Stock/Coffee Info/Publish 膠囊＋上架 toast）②Alo Village Cold Fermentation 處置（老闆拍板兩輪）：43 天老豆剩 4.2kg 掛 Off shelf → 一度改名 'Just for brew' 當內用豆，老闆聽完「零機制純手動標籤」後定案**整個拿掉**——roasts 批（51121af0）**已改回原名 Alo Village CF＋bean_id 恢復＋remaining_kg=0**（帳面用完＝所有畫面消失；實體 4.2kg 店內照喝、帳外）；**product_sync CF 列已刪**（菜單/Publish 永不再列；⚠ Square 後台商品本身還在，老闆有空去 Square 刪）；rtl_sold 記憶清空。CF 杯測（dc8c02b0，沒星）沒星沒貨＝Clean-up 舊杯測建議刪清單會接住，老闆之後一鍵清。生豆主檔不動。**系統裡 'Just for brew' 零殘留**。若未來要做「過賞味自動轉內用」機制：新鮮度線 21/42/60 天全是純顯示，可掛提醒卡＋一鍵轉（老闆這輪沒要）。
