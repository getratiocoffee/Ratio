// public-shop v18 — real stock decides Sold out (2026-07-22, hole ③ of the roast-to-publish audit):
//   v17 and earlier only asked Square whether a row was 'paused'. Stock was never consulted, so a
//   coffee that ran out in BOTH the roastery and Crows Nest stayed fully buyable (live example:
//   Alo Village ×2 and Danche v2 sat at 0 kg on the menu). Now a bean with nothing left anywhere
//   comes back sold_out:true and its variations are dropped from allowedVarIds, so checkout
//   rejects it the same way a paused item is rejected.
//   Deliberately keyed by bean NAME only: roasts/transfers do not reliably carry the process, and a
//   missed key match would hide a coffee we can actually sell. Wide beats wrong here.
//   Fail-open: if either stock query errors, the gate switches off and behaviour falls back to v17
//   (better to sell something we might be short of than to empty the shop on a bad query).
//   Subscriptions are exempt — they have no roast batches to count.
// v17 — off-shelf beans stay on the menu as Sold out (2026-07-22 boss):
...
// v18: kg on hand per bean name = roastery batches (remaining_kg, downgraded batches skipped)
// + Crows Nest shelf (transfers still pool/settled). Returns null when either query fails —
// callers then skip the stock gate entirely rather than emptying the shop on a bad read.
async function stockByName(admin: ReturnType<typeof createClient>, names: string[]): Promise<Map<string, number> | null> {
  ...
  try {
    ...
    (rq.data || []).forEach((r: { bean_name?: string; remaining_kg?: number; qc?: string | null }) => {
      if (r.qc && r.qc !== "pass") return; // downgraded batches are not sellable
...
async function catalogForSynced(admin: ReturnType<typeof createClient>) {
  ...
  const rows = sync || [];
  // v18: stock gate — computed once here so GET (Sold out stamp) and POST (checkout guard) agree
  ...
  const outOfStock = (bean: unknown) => {
    if (!stock) return false; // query failed -> gate off (v17 behaviour)
    const b = String(bean || "");
    if (SUB_RE.test(b)) return false; // subscriptions have no batches to count
  ...
  if (varIds.length) {
    ...
    // blocked = off shelf (paused) OR nothing left anywhere (v18). Both stay visible on the menu
    // as Sold out; neither can be bought — their variations never enter allowedVarIds.
  ...
  return { rows, itemOfVar, livePrice, imageIdsOfItem, sizesOfItem, allowedVarIds, outOfStock };
}
  ...
  try {
    if (req.method === "GET") {
      ...
      const beans = rows.map((r: { bean_id: string; variation_id: string | null; price: string | number | null; grams: number | null; status: string; process: string | null }) => {
        ...
        const off = r.status === "paused" || outOfStock(r.bean_id); // v18: empty shelves count as sold out
    ...
    if (String(body.action || "") === "checkout") {
      ...
      if (Array.isArray(body.items) && body.items.length) {
        ...
        // v18: the legacy single-bean path never touched allowedVarIds, so it needs its own stock check
        if (!SUB_RE.test(bean)) {
          const st = await stockByName(admin, [bean]);
          if (st && (st.get(bean.trim().toLowerCase()) || 0) <= 0) return json({ error: "this coffee is sold out" }, 400);
        }
